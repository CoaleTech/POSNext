...
from frappe.utils.print_format import download_pdf as get_pdf
from frappe.utils.file_manager import save_file


def get_reserved_qty(item_code, warehouse):
	reserved_qty = frappe.db.sql("""select sum(reserved_qty) from `tabBin` where item_code = %s and warehouse = %s""", (item_code, warehouse))
	return reserved_qty[0][0] if reserved_qty and reserved_qty[0][0] else 0


def get_stock_availability(item_code, warehouse):
	actual_qty = get_stock_balance(item_code, warehouse, with_valuation_rate=False)
	reserved_qty = get_reserved_qty(item_code, warehouse)
	return max(actual_qty - reserved_qty, 0), frappe.db.get_value("Item", item_code, "is_stock_item")
