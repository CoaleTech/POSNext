...
@frappe.whitelist()
def get_rooms_and_tables(pos_profile=None):
	"""
	Get rooms and tables for restaurant POS
	"""
	rooms = frappe.get_all(
		"POSNext Room",
		fields=["name", "room_name", "capacity", "printer_settings"]
	)

	for room in rooms:
		tables = frappe.get_all(
			"POSNext Table",
			filters={"restaurant_room": room.name},
			fields=["name", "table_name", "capacity", "status", "restaurant_room"]
		)
		room["tables"] = tables
...
def get_table_details(table_name):
	...
	table = frappe.get_doc("POSNext Table", table_name)
	...
	active_orders = frappe.get_all(
		...
		fields=["name", "invoice", "customer_name", "creation", "type"]
	)
...
def update_table_status(table_name, status):
	...
	table.status = status
	table.save()
	return {"success": True, "message": f"Table {table_name} status updated to {status}"}


@frappe.whitelist()
def generate_kot_on_invoice_save(invoice_name, table_name=None, customer_name=None):
	"""
	Generate KOT when POS invoice is saved
	"""
	if not invoice_name:
	...
	# Check if KOT is enabled for this POS Profile
	pos_profile = frappe.get_doc("POS Profile", invoice.pos_profile)
	if not pos_profile.get("custom_enable_kot"):
		return {"success": True, "message": "KOT not enabled for this POS Profile"}

	# Get current items from invoice
	current_items = []
	...
	# For new invoice, previous items are empty
	previous_items = []

	# Generate KOT
	from posnext.posnext.api.kot_generate import kot_execute
	try:
		kot_execute(
			invoice_name,
			customer_name or invoice.customer,
			table_name,
			current_items,
			previous_items,
			comments=""
		)
		return {"success": True, "message": "KOT generated successfully"}
	except Exception as e:
		frappe.log_error(f"KOT Generation Error: {str(e)}", "KOT Generation")
		return {"success": False, "message": f"Failed to generate KOT: {str(e)}"}


def get_context(context):
	"""
	Set up the context for the POSNext page
	"""
	context.no_cache = 1
	context.show_sidebar = 0
	context.title = "POSNext"
	
	# Add any additional context variables here
	context.posnext_settings = frappe.get_single("POSNext Settings") if frappe.db.exists("DocType", "POSNext Settings") else None
